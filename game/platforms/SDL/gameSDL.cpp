...
class GameSceneHandler :
    public SceneHandlerGL, public MouseHandlerGL, public KeyboardHandlerGL, public JoystickHandlerGL,
	...
	public:
        ...
        // implements the JoystickHandlerGL interface
        virtual void joystickMoved( int num, int inX, int inY );
...
int mainFunction( int inNumArgs, char **inArgs ) {
    ...
    screen =
        new ScreenGL( screenWidth, screenHeight, fullscreen, targetFrameRate,
                      ...
                      customData,
                      hashSalt,
                      getWindowTitle(), NULL, NULL, NULL, NULL );
    ...
    */
    ...
    if( demoMode ) {    
        ...
        screen->addKeyboardHandler( sceneHandler );
        screen->addJoystickHandler( sceneHandler );
...
void GameSceneHandler::joystickMoved( int num, int inX, int inY ) {
    joystickMove(num, inX, inY);
}
...
#ifdef PANDORA
    // quit via select button
    if (mPaused && inKey == 115)
        exit(0);

    // start button simulates an escape for pause menu
    if (inKey == 117) {
        keyPressed(27, inX, inY);
        keyReleased(27, inX, inY);
    }
#endif
